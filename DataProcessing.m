...
for zn = 11;  % Change to match the number of folders
clc
sgt = ['data/Activefolder' sprintf(num2str(zn))];
for g=1:7    
if g <= 3;
        str = [sgt '/Input/trainingdata/level'];
elseif g==4;
        str = [sgt '/Input/paths/point_d'];
elseif g==5;
        str = [sgt '/Input/paths/point_h'];
elseif g==6;
        str = [sgt '/Input/paths/point_v'];
elseif g==7
        str = [sgt '/Input/paths/test'];
end
...
if g<=3; % this number depends on the number of levels I have created in FEA
    ...
    str = '/Output/sampling/level'; 
    str2 = [sgt str sprintf(num2str(g)) st];
%     save(str2,'input_sl');
else
    str = '/Output/data';
    str2 = [sgt str char(lab1{g-3}) st];
end
